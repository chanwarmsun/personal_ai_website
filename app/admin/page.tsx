...
export default function AdminPage() {
  ...
  const compressImage = async (file: File): Promise<string> => {
    const { ImageOptimizer, PerformanceMonitor } = await import('../lib/performance-optimizer')
    
    const timer = PerformanceMonitor.start('image_compression')
    try {
      const result = await ImageOptimizer.smartCompress(file)
      const duration = timer()
      console.log(`📸 图片压缩完成，耗时: ${duration.toFixed(2)}ms`)
      return result
    } catch (error) {
      timer()
  ...
  const loadStats = async () => {
    try {
      setIsLoading(true)
      console.log('🔄 开始轻量级统计数据加载...')
      
      // 尝试从内存缓存快速获取
      const cachedStats = sessionStorage.getItem('admin_stats_cache')
      const cacheTime = sessionStorage.getItem('admin_stats_cache_time')
      ...
      // 缓存过期或不存在，串行加载以减少数据库压力
      console.log('🔄 开始从数据库获取统计数据...')
      
      try {
        // 1. 先加载自定义内容数量（轻量级查询）
        const [customStats, defaultContentData] = await Promise.all([
          // 并发获取自定义内容数量（使用count查询更高效）
          Promise.all([
            supabase.from('agents').select('*', { count: 'exact' }).then(r => r.count || 0),
            supabase.from('prompts').select('*', { count: 'exact' }).then(r => r.count || 0),
            supabase.from('teaching_resources').select('*', { count: 'exact' }).then(r => r.count || 0),
            supabase.from('custom_requests').select('*', { count: 'exact' }).then(r => r.count || 0)
          ]),
          // 2. 获取默认内容（优化查询）
          (async () => {
            try {
              ...
              if (dbContent) {
                return {
                  agents: (dbContent.agents || []).length,
                  prompts: (dbContent.prompts || []).length,
                  teachingResources: (dbContent.teachingResources || dbContent.resources || []).length
                }
              }
              ...
              if (response.ok) {
                ...
                return {
                  agents: (fileData.agents || []).length,
                  prompts: (fileData.prompts || []).length,
                  teachingResources: (fileData.teachingResources || []).length
                }
              }
              
              return { agents: 0, prompts: 0, teachingResources: 0 }
            } catch (error) {
        ...
        console.log('📊 自定义内容数量:', customStats)
        console.log('📋 默认内容数量:', defaultContentData)
        
        // 2. 计算总数量（默认内容 + 自定义内容）
        const totalStats = {
          agents: defaultContentData.agents + customStats[0],
          prompts: defaultContentData.prompts + customStats[1], 
          resources: defaultContentData.teachingResources + customStats[2],
          requests: customStats[3] // 定制申请只统计数据库中的
        }
        
        console.log('📈 总统计数据 (默认+自定义):', totalStats)
        setStats(totalStats)
        
        // 缓存统计数据到sessionStorage
        sessionStorage.setItem('admin_stats_cache', JSON.stringify(totalStats))
        sessionStorage.setItem('admin_stats_cache_time', Date.now().toString())
        console.log('💾 统计数据已缓存')
        
      } catch (dbError) {
        console.error('❌ 数据库查询失败，使用默认统计:', dbError)
        // 如果数据库查询失败，设置默认值
        const fallbackStats = { agents: 0, prompts: 0, resources: 0, requests: 0 }
  ...
  useEffect(() => {
    ...
    const initializeAdmin = async () => {
      console.log('🚀 开始快速初始化管理后台...')
      const startTime = Date.now()
      
      try {
        // 1. 只进行最基础的连接检查（不等待结果）
        checkConnectionStatus().catch(console.warn)
        ...
        console.log(`⚡ 管理后台快速初始化完成，耗时: ${Date.now() - startTime}ms`)
      } catch (error) {
  ...
  useEffect(() => {
    ...
    // 超轻量级初始化：只检查登录，其他数据完全懒加载
    const initializeAuth = async () => {
      console.log('🔐 检查认证状态...')
      
      try {
        // 只进行最基础的数据库连接检查（异步，不阻塞）
        setTimeout(() => {
          checkDatabaseConnection().catch(console.warn)
        }, 1000)
        
        // 完全移除预加载，所有数据都在用户操作时加载
        console.log('✅ 认证检查完成，数据将按需加载')
  ...
  const handleImage = async (e: any) => {
    ...
    try {
      ...
      console.log(`📸 图片智能压缩完成: ${originalSizeMB}MB → ${compressedSizeMB}MB`)
